...
EOF;
    ...
    public static function editField($data, $fieldid = 0)
    {
        ...
        $tablename = self::get_table_prefix() . $tablename;
        //判断字段名唯一性
        if (self::where('name', $data['name'])->where('moduleid', $moduleid)->where('id', '<>', $fieldid)->value('id')) {
            throw new \Exception(lang("field'" . $data['name'] . "`is already exist"));
        }
        $sql = <<<EOF
            ALTER TABLE `{$tablename}`
...
EOF;
    ...
    {
        $table = Module::find($moduleid)->value('name');
        return $table;
    }
